...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 2. [추가] 공지사항 롤링 바 및 상세 모달 인터랙션 관리 모듈
    const NoticeManager = {
        ...
        // 상세 화면 렌더링 (마스터 권한 시 실시간 인라인 편집 지원)
        renderDetail: function(idx) {
            ...
            body.innerHTML = `
                <div class="f3i-ndetail-wrapper">
                    <h3 id="f3iEditTitle" class="${isMasterMode ? 'f3i-nedit-target' : ''}" 
                        contenteditable="${isMasterMode}">${targetNotice.title}</h3>
                    ...
                    <div id="f3iEditContent" class="f3i-ndetail-content ${isMasterMode ? 'f3i-nedit-target' : ''}" 
                         contenteditable="${isMasterMode}">${targetNotice.content}</div>
                </div>
            `;
        },

        // 마스터 모드에서 상단 [저장]버튼 클릭 연동 시 데이터를 취합하여 영구저장
